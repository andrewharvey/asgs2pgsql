...
CREATE OR REPLACE FUNCTION asgs_2016.match_abs_structure_code(code character varying(11)) RETURNS text AS $$
BEGIN
  IF code ~ E'^\\d$' THEN
      return 'ste';
...
-- For a given ASGS_2016.LGA name return a short version of the name
CREATE OR REPLACE FUNCTION asgs_2016.shorten_lga_name(name text) RETURNS text AS $$
BEGIN
  return regexp_replace(name, E' \\(.*', '');
END
$$ LANGUAGE plpgsql;

-- For a given ASGS_2016.LGA name return the LGA status type code
CREATE OR REPLACE FUNCTION asgs_2016.lga_status_type_code(name text) RETURNS text AS $$
BEGIN
  return (regexp_matches(name, E'\\((.*)\\)'))[1]::text;
END
$$ LANGUAGE plpgsql;

-- For a given ASGS_2016.LGA status type code return the full name for that Status Type
CREATE OR REPLACE FUNCTION asgs_2016.lga_status_type_code_to_name(code text) RETURNS text AS $$
BEGIN
  CASE code
    WHEN 'C' THEN
      return 'City';
