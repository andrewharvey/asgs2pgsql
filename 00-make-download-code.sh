...
do
  ...
  do
      curl -o - "http://www.abs.gov.au/AUSSTATS/abs@.nsf/DetailsPage/1270.0.55.00${i}July%20${year}?OpenDocument" 2> /dev/null | grep --binary-files=text --ignore-case --only-matching '/ausstats[^"]*' | grep -E '(Latest|Previous)$' | grep \.zip | grep --invert-match 'midmif\.zip' | grep --invert-match 'tab\.zip' | cut -d'&' -f2,5,7,8,9 | sed 's/&/" "/g' | sed 's/$/"/' | sed "s/^127005500${i}_/download ${i} \"/" | sed "s/^cg_/download cg \"/"
    echo ""
  done
done
